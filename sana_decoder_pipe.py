...
class SanaPipeline(DiffusionPipeline, SanaLoraLoaderMixin):
    ...
    model_cpu_offload_seq = "text_encoder->transformer->vae->vq_model_embedder"
    ...
    def __init__(
        ...
        if self.text_encoder is not None:
            empty_prompt_embeds, empty_prompt_attention_mask, negative_prompt_embeds, negative_prompt_attention_mask = self.encode_prompt(
                '')
            self.empty_prompt_embeds = empty_prompt_embeds
            self.empty_prompt_attention_mask = empty_prompt_attention_mask
    ...
    def __call__(
        ...
        if self.vq_embed_projector is not None:
            ...
            prompt_attention_mask = torch.ones(prompt_embeds.shape[:2], dtype=torch.long)
            print('using vq_embedder projector')
        else:
            prompt_embeds = self.empty_prompt_embeds.repeat(vq_embeds.shape[0], 1, 1)
            prompt_attention_mask = self.empty_prompt_attention_mask.repeat(vq_embeds.shape[0], 1)
        ...
        # 5. Prepare latents.
        latent_channels = self.transformer.config.in_channels - self.dualvitok_channels
        ...
        # 5.5. Process image embeddings to match latent dimensions
        # Transform image embeds to match latent dimensions
        latent_height = int(height) // self.vae_scale_factor
        latent_width = int(width) // self.vae_scale_factor
        if vq_embeds.shape[-2:] != (latent_height, latent_width):
            vq_embeds = torch.nn.functional.interpolate(
                vq_embeds.float(),
                size=(latent_height, latent_width)
            ).to(vq_embeds.dtype)

        # Duplicate for classifier-free guidance if needed
        # if self.do_classifier_free_guidance:
        #     vq_embeds = torch.cat([latents] * 2)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print(vq_embeds.mean(), vq_embeds.std())
                ...
                noise_pred = self.transformer(
                    latent_model_input.to(dtype=transformer_dtype, device=device),
                    encoder_hidden_states=prompt_embeds.to(dtype=transformer_dtype, device=device),
                    encoder_attention_mask=prompt_attention_mask.to(dtype=transformer_dtype, device=device),
                    timestep=timestep,
                    return_dict=False,
                ...
                if self.do_classifier_free_guidance:
                    ...
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_image - noise_pred_uncond)
